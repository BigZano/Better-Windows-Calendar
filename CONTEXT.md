...
Microsoft Graph (Outlook) is now reachable end-to-end too. The initial OAuth login uses the **loopback redirect + PKCE (S256)** flow (ADR-0008), implemented in the `internal/graphauth` package and surfaced as **Connect Outlook** in the Add Calendar dialog: choosing the **Outlook** Calendar type reveals a "Connect Microsoft account" button that runs `graphauth.Login` (open the system browser → 127.0.0.1 loopback callback → exchange the code for tokens), then `msgraph.ListCalendars` lists the account's Outlook calendars for the user to pick. On Save the picked Graph calendar ID is stored as the Calendar's `sync_url`, the refresh token goes to the CredentialStore, and the Calendar registers with the live engine. This **needs a user-supplied `microsoft_client_id`** in `config.toml` (no built-in default ships — see `docs/azure-app-registration.md`); the dialog refuses to connect until it is set. ADR-0008 supersedes ADR-0005's OAuth-callback mechanism (custom `pycalendar://` URI scheme, single-instance mutex, named-pipe IPC) for the OAuth path. The single-instance machinery now lives on as the `.ics` **file-association launch** path (ADR-0009): a Windows-only named mutex (single-instance authority) + named pipe (arg channel) in `internal/singleinstance`, used in tray mode only. `.ics` file import is implemented as an in-app Import Dialog (see below); double-click launch is now wired through that single-instance path (see the Import Dialog section).
...
**`.ics` double-click is now wired** (Windows): the installer registers a `PyCalendar.ics` ProgID + `.ics` association (`setup.iss`), so opening a `.ics` via "Open with" launches `pycalendar.exe "<path>"`. `main.go` detects the positional `.ics` argument (`firstICSArg`) in **tray mode only** and routes it through **single-instance** (`internal/singleinstance`): the running tray (primary) opens the Import Dialog for the path via `ui.OpenImportPath`; a secondary launch forwards the path over the named pipe and exits. Single-instance is **Windows-only** (named mutex as authority + named pipe as the arg channel) and **tray-only** — `bar`/`cli` never touch the mutex or pipe. On non-Windows every launch is the primary (no-op). See ADR-0009. UserChoice blocks forcing the *default* handler, so association is opt-in via "Open with".
