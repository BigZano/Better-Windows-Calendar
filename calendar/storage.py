...
def get_connection(max_retries: int = 5) -> sqlite3.Connection:
    ...
    for attempt in range(max_retries):
        try:
            ...
            conn.execute("PRAGMA bus_timeout=5000")
...
def init_db():
    ...
    if current_version < SCHEMA_VERSION:
        cursor.execute("INSERT INTO schema_version (version, applied_at) VALUES (?, ?)", (SCHEMA_VERSION, int(time.time())))
        conn.commit()
        conn.close()

def execute_query(query: str, params: tuple = (), fetch: bool = True) -> Optional[List[sqlite3.Row]]:
    conn = get_connection()
    cursor = conn.cursor()
